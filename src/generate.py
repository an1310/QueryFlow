...
class BasicGenerator:
    ...
    def __init__(self, model_name_or_path):
        ...
        # Handle Llama models differently since they don't use config.json
        if "llama" in model_name_or_path.lower():
            from transformers import LlamaTokenizer, LlamaForCausalLM
            self.tokenizer = LlamaTokenizer.from_pretrained(model_name_or_path, legacy=True)
...
class AttnWeightRAG(BasicRAG):
    ...
    def keep_real_words(self, prev_text, curr_tokens, curr_hit):
        """
        Analyzes text to identify and retain reliable content words based on attention patterns.

        Args:
            prev_text (str): Previous context text
            curr_tokens (list): Current tokens being analyzed
            curr_hit (list): Binary list marking potentially hallucinated tokens (1 = hallucinated)

        Returns:
            str: Space-joined string of reliable content words
        """
        # Combine current tokens into text and append to previous context
        curr_text = " ".join(curr_tokens)
        all_text = prev_text + " " + curr_text

        # Convert text to model input tensors and move to GPU
        input_ids = self.generator.tokenizer.encode(all_text, return_tensors="pt")
        input_ids = input_ids.to(self.generator.model.device)
        input_length = input_ids.shape[1]

        # Get the raw tokens from the tokenizer
        tokens_tmp = self.generator.tokenizer.convert_ids_to_tokens(input_ids[0])

        # Get attention patterns from the model's last layer
        ...
        # Each range_ entry contains [start_idx, end_idx] for a complete word
        range_ = []
        for i, t in enumerate(tokens_tmp):
            if (i == 0 or  # First token
                    t.startswith(self.generator.space_token) or  # New word with space
                    input_ids[0][i] == 13 or  # Carriage return
                    tokens_tmp[i - 1] == '</s>'):  # End of sequence token
                range_.append([i, i])  # Start new word
            else:
                range_[-1][-1] += 1  # Extend current word

        # Convert token ranges back to complete words
        tokens = []
        for r in range_:
            tokenseq = "".join(tokens_tmp[r[0]: r[1] + 1]).replace(self.generator.space_token, "")
            tokens.append(tokenseq)

        # Find where current tokens start in the complete token list
        curr_st = len(tokens) - len(curr_tokens)

        # Average attention across heads and move to CPU for processing
        atten_tmp = torch.mean(atten_tmp, dim=0).cpu()

        # Calculate attention patterns for each word
        attns = []
        for r in range_:
            # Initialize attention tensor for this word
            att = torch.zeros(input_length)

            # For each token in this word
            for i in range(r[0], r[1] + 1):
                if i == 0:  # Skip first token (usually BOS)
                    continue

                # Get attention to previous tokens and normalize
                v = atten_tmp[i - 1][:r[0]]  # Attention weights to previous tokens
                v = v / v.sum()  # Normalize to sum to 1

                # Create attention map for this token
                t = torch.zeros(input_length)
                t[:r[0]] = v
                att += t

            # Average attention across all tokens in this word
            att /= (r[1] - r[0] + 1)

            # Sum attention values for each complete word
            att = torch.tensor([att[rr[0]:rr[1] + 1].sum() for rr in range_])
            attns.append(att)

        # Calculate forward attention weights for hallucinated tokens
        forward_attns = torch.zeros(len(tokens))
        hit_cnt = 0
        for i in range(len(curr_hit)):
            if curr_hit[i] == 1:  # If token is marked as hallucinated
                forward_attns += attns[curr_st + i]  # Add its attention pattern
                hit_cnt += 1

        if hit_cnt > 0:  # Avoid division by zero
            forward_attns /= hit_cnt
        forward_attns = forward_attns.tolist()

        # Use spaCy to identify content words (nouns, adjectives, verbs, etc.)
        doc = nlp(all_text)
        real_words = set(token.text for token in doc if token.pos_ in
                         ['NOUN', 'ADJ', 'VERB', 'PROPN', 'NUM'])

        # Helper function to check if a token contains a content word
        def match(token):
            for word in real_words:
                if word in token:
                    return True
        ...
        # Build list of (attention_score, token, position) for content words
        real_pairs = []
        for i in range(len(tokens)):
            tok, att = tokens[i], forward_attns[i]
            # Skip if token is in current sequence and marked as hallucinated
            if i >= curr_st and curr_hit[i - curr_st]:
                continue
            if match(tok):
                real_pairs.append((att, tok, i))

        # Determine how many top words to keep
        ...
        # Sort by attention score, take top k, then resort by position
        real_pairs = sorted(real_pairs, key=lambda x: x[0], reverse=True)  # Sort by attention
        real_pairs = real_pairs[:top_k]  # Take top k
        real_pairs = sorted(real_pairs, key=lambda x: x[2])  # Sort by position

        # Return space-joined string of selected words
    ...
    def inference(self, question, demo, case):
        ...
        while True:
            ...
            new_text, tokens, attns, logprobs, entropies = self.generator.generate_attn(
                ...
                use_entropy = self.method == "queryflow",
                use_logprob = self.method == "attn_prob"
            )
            weight = entropies if self.method == "queryflow" else [-v for v in logprobs]
